...
It may happen that there is an error in scraping the selected URL. In such a case, tell the reviewer to tell the router to choose 
next agent as selector.
...
reviewer_prompt_template = """
You are a reviewer. Your task is to review the reporter's response to the research question and provide feedback.
If an agent has clearly mentioned relevant source(s) but you still think it has not provided citations, then tell 
them to write "Citations are provided below" when mentioning sources. In this case, now you can set citations_provided to be True,
but remember that the sources have to be relevant.
...
It may happen that there is an error in scraping the selected URL. In such a case, tell the router to choose 
next agent as selector.
...
If the prompt is about Tata Steel and comprehensive is true and relevant_to_research_question is true and pass_review is true, then select the next agent as final_report.
